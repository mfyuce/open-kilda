...
public class BroadCastDiscoveryRuleGeneratorTest {
    ...
    private void checkGroupCommand(GroupSpeakerCommandData groupCommandData) {
        ...
        buckets.forEach(b -> assertEquals(WatchGroup.ANY, b.getWatchGroup()));

        Bucket first = buckets.get(0);
        assertEquals(2, first.getActions().size());
        SetFieldAction setEthDstAction = getActionByType(SetFieldAction.class, first.getActions());
        assertEquals(Field.ETH_DST, setEthDstAction.getField());
        assertEquals(sw.getSwitchId().toLong(), setEthDstAction.getValue());
        PortOutAction sendToControllerAction = getActionByType(PortOutAction.class, first.getActions());
        ...
        SetFieldAction setUdpDstAction = getActionByType(SetFieldAction.class, second.getActions());
        assertEquals(Field.UDP_DST, setUdpDstAction.getField());
        assertEquals(LATENCY_PACKET_UDP_PORT, setUdpDstAction.getValue());
        PortOutAction sendToPortInAction = getActionByType(PortOutAction.class, second.getActions());
